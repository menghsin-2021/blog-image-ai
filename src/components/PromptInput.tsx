...
export const PromptInput: React.FC<PromptInputProps> = ({
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      <label className="block text-sm font-medium text-gray-700">圖片描述</label>
      ...
      <div className="relative">
        ...
        <div className="absolute bottom-3 right-3">
          <span className="text-xs text-gray-400">{characterCount} 個字元</span>
        </div>
      </div>
      ...
      <div className="text-xs text-gray-500 space-y-1">
        ...
        <p>
          📝
          範例：「現代扁平風格的雲端運算示意圖，藍色調，包含伺服器、網路連線和資料傳輸元素，背景是漸層藍色，整體風格簡潔專業，適合技術部落格使用」
        </p>
      </div>

      {/* 快速建議標籤 */}
      <div className="flex flex-wrap gap-2">
        {['技術示意圖', '程式碼概念', '資料流程', '系統架構', '使用者介面', '網路拓撲'].map(
          suggestion => (
            <button
              key={suggestion}
              onClick={() => {
