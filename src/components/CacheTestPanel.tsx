...
import {
  ...
  useContainerResourceMonitor,
  CachePerformanceMetrics,
} from '../utils/containerCacheTest';
...
export function CacheTestPanel() {
  ...
  const runRealCacheTest = async () => {
    ...
    try {
      ...
      for (let i = 0; i < testDataSet.length; i++) {
        ...
        try {
          ...
          if (!result) {
            ...
            addLog(
              `chinese: ${result.optimized.chinese ? '✅' : '❌'}, english: ${result.optimized.english ? '✅' : '❌'}`
            );
          } else {
      ...
      addLog(
        `📊 快取統計 - 項目: ${cacheStats.size}, 命中: ${cacheStats.hits}, 未命中: ${cacheStats.misses}`
      );
    } catch (error) {
      addLog(`❌ 真實測試失敗: ${error}`);
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <div className="border-b border-gray-200 pb-4 mb-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">🐳 容器化快取系統測試面板</h2>
        <p className="text-gray-600">測試 Docker 容器環境中的快取效能和功能</p>
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white p-3 rounded border">
              ...
              <div className="text-2xl font-bold text-green-600">
                {testResults.averageResponseTime}ms
              </div>
            </div>
            <div className="bg-white p-3 rounded border">
      ...
      <div className="mt-6 bg-blue-50 rounded-lg p-4">
        ...
        <ul className="text-blue-800 text-sm space-y-1">
          <li>
            • <strong>效能測試</strong>: 模擬快取操作，測量回應時間和命中率
          </li>
          <li>
            • <strong>真實功能測試</strong>: 實際呼叫 GPT-4o API 和快取系統
          </li>
          <li>
            • <strong>容器環境</strong>: 所有測試在 Docker 容器中執行
          </li>
          <li>
            • <strong>效能目標</strong>: 命中率 &gt;70%, 回應時間 &lt;300ms, 記憶體 &lt;100MB
          </li>
        </ul>
      </div>
    </div>
