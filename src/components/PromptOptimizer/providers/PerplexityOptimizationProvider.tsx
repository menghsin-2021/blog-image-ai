...
export class PerplexityOptimizationProvider implements OptimizationProviderInterface {
  ...
  async optimize(
    ...
    try {
      ...
      // 選擇模型
      const selectedModel = this.convertModelName(options?.model || 'llama-3.1-sonar-large-128k-online');
      ...
      // 轉換為統一格式
      return this.convertToUnifiedResult(result, options?.model || 'llama-3.1-sonar-large-128k-online');
    } catch (error) {
