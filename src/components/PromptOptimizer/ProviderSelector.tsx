...
export const ProviderSelector: React.FC<ProviderSelectorProps> = ({
  ...
  const renderPerplexityModelSelector = () => {
    ...
    return (
      <div className="mt-4 space-y-3">
        <label className="block text-sm font-medium text-gray-700">選擇 Perplexity 模型</label>
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h4 className="font-medium text-gray-900">{modelInfo.name}</h4>
                        <p className="text-sm text-gray-600 mt-1">{modelInfo.description}</p>
                      </div>
                    </div>
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-3">選擇 AI 最佳化服務</label>
        <div className="grid gap-3">
      ...
      {estimatedCost !== undefined && (
        <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <p className="text-xs text-blue-700 mt-1">實際成本可能根據內容長度和複雜度有所變化</p>
        </div>
      )}
    </div>
  );
