  ...
  banner: [
    {
      ...
      purpose: 'banner',
      template:
        'A modern, professional banner image representing {MAIN_CONCEPT}, featuring {VISUAL_ELEMENTS}, {STYLE_MODIFIERS}, suitable for a tech blog header',
    ...
    },
    {
      ...
      purpose: 'banner',
      template:
        'An eye-catching banner image about {TOPIC}, {STYLE_MODIFIERS}, perfect for blog header, {VISUAL_ELEMENTS}',
  ...
  illustration: [
    {
      ...
      template:
        'A simple, clear illustration explaining {CONCEPT}, {STYLE_MODIFIERS}, educational diagram style, {VISUAL_ELEMENTS}',
    ...
    },
    {
      ...
      purpose: 'illustration',
      template:
        'A step-by-step visual guide showing {PROCESS}, {STYLE_MODIFIERS}, flowchart style, {VISUAL_ELEMENTS}',
  ...
  summary: [
    {
      ...
      purpose: 'summary',
      template:
        'A conceptual image summarizing {MAIN_THEME}, {STYLE_MODIFIERS}, symbolic representation, {VISUAL_ELEMENTS}',
    ...
    },
    {
      ...
      purpose: 'summary',
      template:
        'An inspiring image representing the conclusion of {TOPIC}, {STYLE_MODIFIERS}, thought-provoking visual, {VISUAL_ELEMENTS}',
      placeholders: ['TOPIC', 'STYLE_MODIFIERS', 'VISUAL_ELEMENTS'],
...
function selectVisualElements(analysis: ContentAnalysis, purpose: ImagePurposeType): string[] {
  ...
  if (analysis.technicalTerms.length > 0) {
    if (
      analysis.technicalTerms.some(term => ['ai', '人工智慧', 'machine learning'].includes(term))
    ) {
    ...
    if (
      analysis.technicalTerms.some(term =>
        ['javascript', 'python', 'code', '程式碼'].includes(term)
      )
    ) {
...
function buildOptimizedPrompt(
  ...
  // 添加通用的品質指引
  englishPrompt +=
    ', high quality, professional photography, clean composition, suitable for blog use';
 ...
 * 生成最佳化建議
 */
function generateSuggestions(analysis: ContentAnalysis, purpose: ImagePurposeType): string[] {
 ...
 * 計算最佳化信心度
 */
function calculateConfidence(content: ContentInput, analysis: ContentAnalysis): number {
...
export async function optimizeForAllPurposes(
  ...
  const results = await Promise.all(purposes.map(purpose => optimizePrompt(content, purpose)));
