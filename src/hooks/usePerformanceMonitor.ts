...
export const usePerformanceMonitor = () => {
  ...
  // 測量提示詞最佳化時間
  const measurePromptOptimization = useCallback(
    async <T>(optimizationFn: () => Promise<T>): Promise<T> => {
      const startTime = performance.now();
  ...
  // 測量圖片生成時間
  const measureImageGeneration = useCallback(
    async <T>(generationFn: () => Promise<T>): Promise<T> => {
      const startTime = performance.now();
