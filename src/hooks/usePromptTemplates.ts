...
import { PromptTemplate, PromptTemplateCategory, TemplateUsage, UsePromptTemplatesReturn } from '../types/promptTemplates';
import { BUILTIN_TEMPLATES, TEMPLATE_CATEGORIES, TEMPLATE_SEARCH_KEYWORDS } from '../utils/promptTemplates';
...
export const usePromptTemplates = (): UsePromptTemplatesReturn => {
  ...
  const saveToStorage = useCallback((data: Partial<StoredTemplateData>) => {
    try {
      ...
      const existingData: StoredTemplateData = currentData ? JSON.parse(currentData) : {
        customTemplates: [],
        recentUsage: [],
        favorites: [],
  ...
  // 依分類取得模板
  const getTemplatesByCategory = useCallback((category: PromptTemplateCategory): PromptTemplate[] => {
  ...
  // 使用模板
  const useTemplate = useCallback((templateId: string, variables: Record<string, string>): string => {
  ...
  // 自訂模板管理
  const createTemplate = useCallback((templateData: Omit<PromptTemplate, 'id' | 'createdAt' | 'updatedAt' | 'usageCount'>) => {
