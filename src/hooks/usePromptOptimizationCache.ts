...
export const usePromptOptimizationCache = () => {
  ...
  // 快取的提示詞最佳化
  const optimizePromptWithCache = useCallback(
    async (
  ...
  const preloadCommonOptimizations = useCallback(async () => {
    const commonCombinations = [
      {
        purpose: 'hero-banner' as ImagePurposeType,
        content: { title: '', content: '', keywords: [] },
      },
      {
        purpose: 'section-illustration' as ImagePurposeType,
        content: { title: '', content: '', keywords: [] },
      },
      {
        purpose: 'content-summary' as ImagePurposeType,
        content: { title: '', content: '', keywords: [] },
      },
    ];
  ...
  const isCached = useCallback(
    (content: ContentInput, purpose: ImagePurposeType): boolean => {
      const cacheKey = generateCacheKey(content, purpose);
  ...
  const setCachedResult = useCallback(
    (content: ContentInput, purpose: ImagePurposeType, result: OptimizedPrompt) => {
      const cacheKey = generateCacheKey(content, purpose);
      cache.set(cacheKey, result);
    },
    [cache]
