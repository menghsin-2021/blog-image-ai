...
export const getAspectRatiosForModel = (model: DalleModel): AspectRatio[] => {
  switch (model) {
    ...
    case 'gpt-image-1':
      // GPT-image-1 支援不同的尺寸：1024x1024, 1536x1024, 1024x1536
      return ASPECT_RATIOS.filter(ratio => 
        ['1024x1024', '1536x1024', '1024x1536'].includes(ratio.value)
      );
    default:
...
export const IMAGE_QUALITIES: { value: ImageQuality; label: string; models: DalleModel[] }[] = [
  ...
  { value: 'hd', label: '高畫質 (HD)', models: ['dall-e-3'] } // GPT-image-1 使用 high/medium/low
];
